...
class Zappa:
        ...
        if use_precompiled_packages:
            ...
            for (
                ...
                cached_wheel_path = self.get_cached_manylinux_wheel(
                    installed_package_name, installed_package_version, disable_progress
                )
    ...
    def download_url_with_progress(url, stream, disable_progress):
        ...
        """
        resp = requests.get(
            url, timeout=float(os.environ.get("PIP_TIMEOUT", 10)), stream=True
        )
    ...
    def update_lambda_configuration(
        ...
        print({"Variables": aws_environment_variables})
